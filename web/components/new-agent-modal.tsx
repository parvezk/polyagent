      ...
      <DialogContent className="border-zinc-700/60 bg-zinc-900 text-zinc-100 sm:max-w-[40rem]">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-1.5">
            ...
            <div role="radiogroup" aria-labelledby="vendor-label" className="grid grid-cols-4 gap-2">
              {VENDORS.map((v) => {
          ...
          {/* Repo (+ branch) */}
          <div className="space-y-1.5">
            <label id="repo-label" className="text-xs font-medium text-zinc-400">
              Repo {repoRequired(vendor) ? "" : <span className="text-zinc-600">(optional)</span>}
            </label>
            {vendor === "jules" ? (
              <Select value={repo} onValueChange={(v) => v && setRepo(v)} aria-labelledby="repo-label">
                <SelectTrigger aria-labelledby="repo-label" className="border-zinc-700 bg-zinc-950/50">
          ...
          {vendor === "claude" && (
            <div className="space-y-1.5">
              <label id="model-label" className="text-xs font-medium text-zinc-400">Model</label>
              <Select value={model} onValueChange={(v) => v && setModel(v)} aria-labelledby="model-label">
                <SelectTrigger aria-labelledby="model-label" className="border-zinc-700 bg-zinc-950/50">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="border-zinc-700 bg-zinc-900 text-zinc-100">
                  {CLAUDE_MODELS.map((m) => (
          ...
          <div className="space-y-1.5">
            <label id="task-label" className="text-xs font-medium text-zinc-400">Task</label>
            <Textarea
              value={prompt}
