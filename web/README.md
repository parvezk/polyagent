...
| `POST` | `/api/sessions/:id/followup` | RLS | `{ message }` — `400` if empty; vendor errors → `502`; patches status to `running` |
...
### Live polling (list + detail)

Client SWR refreshes the table/telemetry strip every **3s** (`/api/sessions`) and the session drawer every **4s** (`/api/sessions/:id`).

On the server, both list and detail routes:

- **Skip vendor `getStatus` for terminal rows** (`completed` / `failed`) to avoid useless API calls and DB writes.
- **List concurrency:** vendor polls run through `p-limit(5)`.
- **List persistence:** only status changes (or the first successful `last_polled`) are collected, then **bulk `upsertSessions`** once per request.
- Vendor poll failures keep the last-known status (no 5xx for a single bad adapter call).

After a successful follow-up, the route sets status to `running` and updates `last_polled`, so the next poll cycle treats the session as active again.
...
`POST /api/import` reads the CLI state file via `StateStore(STATE_PATH)`, maps rows with `toDbRow`, and **bulk upserts** (`upsertSessions`) to avoid N+1 inserts. Large state files may still hit payload limits (see TODO in route). CLI state is written with directory `0700` / file `0600`.
...
| List endpoint feels chatty / slow | Check non-terminal session count; list polls vendors with concurrency 5 and only upserts changed rows |
| Follow-up succeeded but UI looks idle | Drawer/list SWR should resume after mutate; detail route skips status polls only while status stays `completed`/`failed` |
