 ...
 */
export function colorStatus(status: string, tick = 0): string {
  switch (status) {
    case "running":
 ...
 */
const VENDOR_COLOR: Record<string, (s: string) => string> = {
    ...
    .map((line, rowIdx) =>
      line
        .map((cell, col) => {
          ...
          if (rowIdx === 0) return pc.bold(pc.dim(padded)); // header row
          ...
          if (key === "vendor") {
            const fn = VENDOR_COLOR[cell] ?? ((s: string) => s);
            return fn(cell) + " ".repeat(widths[col] - cell.length);
          }
          if (key === "label") return pc.dim(padded);
          return padded;
        })
        .join(SEP),
    )
