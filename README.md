...
The interactive CLI wizard currently offers **Claude** and **Jules**. Flag-driven CLI dispatch casts `--vendor` to the shared `Vendor` type (all four), though Commander help still lists `claude | jules`. The web UI can target any registered vendor.
...
- **CLI state:** `~/.polyagent/state.json` via `StateStore` — directory `0700`, file `0600` on write
...
Details, auth model, API surface, live polling, and Playwright: [`web/README.md`](web/README.md).
...
- **Repo requirements.** Jules requires `--repo` (enforced in CLI wizard + `POST /api/dispatch`). Cursor also needs a repo in the web New Agent modal; the API currently only hard-requires `repo` for Jules.
...
- **State file permissions.** `StateStore.save()` creates `~/.polyagent` as `0700` and writes `state.json` as `0600`. If you hand-edit the file as another user or with looser modes, expect permission surprises on the next save.
...
| [`web/README.md`](web/README.md) | Dashboard setup, auth, APIs, polling, e2e, troubleshooting |
