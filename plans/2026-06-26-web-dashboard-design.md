...
> **Status:** Shipped and evolved past this plan. The live app uses Supabase auth + RLS (not the JSON file for persistence), includes `/api/import` for CLI→web migration, Cursor/Gemini vendors, and auth on `/api/jules/sources`. See [`web/README.md`](../web/README.md).
